...
router  = APIRouter()
moteur  = MoteurHybride()  # Initialisé au démarrage
...
def recommander(requete: dict, db: Session = Depends(get_db)):

    annonces = fetch_annonces(db)

    moteur = MoteurHybride()

    resultats = moteur.recommander(annonces, requete)
...
def predire_prix(requete: RequetePrixPredit, _=Depends(verifier_api_key)):

    if moteur is None:
        raise HTTPException(status_code=503, detail="Moteur non initialisé")
    ...
    if not prediction or prediction.get("prix_estime") is None:
